...
class ProfileHeaderCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final name = user?.fullName.isNotEmpty == true
        ? user!.fullName
        : (user?.vendorBusinessName.isNotEmpty == true ? user!.vendorBusinessName : 'کاربر');
    return Container(
      ...
      child: Column(
        children: [
          CircleAvatar(
            ...
            child: Text(
              name.isNotEmpty ? name[0] : 'ی',
              style: GetirStyleDeliveryUiTypography.headlineMd(_fa, color: GetirStyleDeliveryUiColors.onPrimary),
            ),
          ),
          const SizedBox(height: 14),
          Text(name, style: GetirStyleDeliveryUiTypography.headlineSm(_fa, color: GetirStyleDeliveryUiColors.onPrimaryFixed)),
...
String roleLabelFa(String? role) => switch (role) {
      'vendor' => 'فروشنده',
      'operator' => 'اپراتور',
      'admin' => 'مدیر',
