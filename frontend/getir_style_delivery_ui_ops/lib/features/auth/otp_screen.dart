...
class _OtpScreenState extends State<OtpScreen> {
  ...
  Future<void> _verify() async {
    ...
    setState(() => _busy = true);
    final ok = await context.read<AuthProvider>().verifyOtp(widget.phone, _code.text.trim());
    if (!mounted) return;
    ...
    if (ok) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(context.read<AuthProvider>().error ?? 'کد نامعتبر')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(backgroundColor: Colors.transparent, foregroundColor: GetirStyleDeliveryUiColors.onPrimary),
      body: SafeArea(
        child: Padding(
            ...
            children: [
                  ...
                  children: [
                    ...
                    if (widget.debugCode != null) ...[
                      ...
                      Container(
                        ...
                        child: Text('کد تست: ${widget.debugCode}',
                            textDirection: TextDirection.ltr,
                            style: GetirStyleDeliveryUiTypography.labelLg(
                                _fa, color: GetirStyleDeliveryUiColors.secondaryContainer)),
                      ),
                    ],
                  ],
                ),
              ...
              TextField(
                ...
                style: GetirStyleDeliveryUiTypography.headlineMd(_fa, color: GetirStyleDeliveryUiColors.onPrimary)
                    .copyWith(letterSpacing: 12, fontWeight: FontWeight.w900),
                decoration: InputDecoration(
                  counterText: '',
                  filled: true,
                  fillColor: GetirStyleDeliveryUiColors.surface,
